...
Component MakeHome() {
  ...
  auto btns = Container::Horizontal(
      {btn_tasks, btn_form, btn_tree, btn_log, btn_wizard});
  ...
  return Renderer(btns, [=]() -> Element {
    ...
    return vbox({
        ...
        text("  Router · DataTable · Tree · Form · Log · Wizard · Palette  ") |
            dim | hcenter,
        separatorEmpty(),
        separator(),
        separatorEmpty(),
        // Live metrics
        hbox({text("  CPU  ") | dim, gauge(m.cpu) | color(t.primary) | flex,
              text(std::to_string(int(m.cpu * 100)) + "% ") |
                  size(WIDTH, EQUAL, 6)}),
        hbox({text("  RAM  ") | dim, gauge(m.ram) | color(t.accent) | flex,
              text(std::to_string(int(m.ram * 100)) + "% ") |
                  size(WIDTH, EQUAL, 6)}),
        separatorEmpty(),
        separator(),
        separatorEmpty(),
        ...
        separatorEmpty(),
        hbox({btn_tasks->Render(), text(" "), btn_form->Render(), text(" "),
              btn_tree->Render(), text(" "), btn_log->Render(), text(" "),
              btn_wizard->Render()}) |
            hcenter,
        separatorEmpty(),
        separator(),
        separatorEmpty(),
        hbox({
            text("  Ctrl+P") | color(t.accent),
            text("=palette  "),
            text("?") | color(t.accent),
            text("=keys  "),
            text("q") | color(t.accent),
            text("=quit  "),
        }) | hcenter |
            dim,
        separatorEmpty(),
...
// ─────────────────────────────────────────────────────────────────────────────
struct TodoItem {
  std::string title, priority, status;
};
...
Component MakeTasks() {
  ...
  auto tbl =
      DataTable<TodoItem>()
          .Column(
              "Title", [](const TodoItem& t) { return t.title; }, 22)
          .Column(
              "Priority", [](const TodoItem& t) { return t.priority; }, 9)
          .Column(
              "Status", [](const TodoItem& t) { return t.status; }, 13)
          .Data(&todos)
          .Selectable(true)
          .Sortable(true)
          .AlternateRows(true)
...
Component MakeForm() {
  ...
  static std::vector<std::string> prio_opts = {"Critical", "High", "Medium",
                                               "Low"};
  ...
  auto frm = Form()
                 ...
                 .Submit("Create Issue",
                         [] {
                           std::string msg =
                               "Created: " +
                               (f_title.empty() ? "(untitled)" : f_title);
...
Component MakeTree() {
  ...
  auto tree =
      Tree()
          ...
          .Node(
              TreeNode::Leaf("CMakeLists.txt", [&] { nav("CMakeLists.txt"); }))
          .Build();
...
Component MakeLog() {
  ...
  return Renderer(lay, [=]() -> Element {
    ...
    return vbox({
        hbox(
            {text("  Log  ") | bold | color(t.primary) | flex, back->Render()}),
        separator(),
...
Component MakeWizard() {
  ...
  auto step1 = Renderer([]() -> Element {
    ...
    return vbox({
        ...
        separatorEmpty(),
        paragraph("This wizard will scaffold a new FTXUI-based project.") |
            xflex,
        separatorEmpty(),
    });
  });
  ...
  auto step3 = Renderer(lay3, [&]() -> Element {
    const Theme& t = GetTheme();
    return vbox({
        text("Options") | bold | color(t.primary),
        separatorEmpty(),
        radio3->Render(),
        separatorEmpty(),
        chk3->Render(),
  ...
  auto step4 = Renderer([&]() -> Element {
    ...
    return vbox({
        text("Review") | bold | color(t.primary),
        separatorEmpty(),
...
int main() {
  ...
  root =
      CommandPalette()
          .Bind("Ctrl+P")
          .Add(
              "Home", [] { g_router->Push("home"); }, "", "Navigate")
          .Add(
              "Tasks", [] { g_router->Push("tasks"); }, "", "Navigate")
          .Add(
              "Form", [] { g_router->Push("form"); }, "", "Navigate")
          .Add(
              "Tree", [] { g_router->Push("tree"); }, "", "Navigate")
          .Add(
              "Log", [] { g_router->Push("log"); }, "", "Navigate")
          .Add(
              "Wizard", [] { g_router->Push("wizard"); }, "", "Navigate")
          .Add(
              "Theme: Default",
              [] {
                SetTheme(Theme::Default());
                Notify("Default", Severity::Info);
              },
              "", "Theme")
          .Add(
              "Theme: Dark",
          ...
          .Add(
              ...
              [] {
                ...
                Notify("Monokai", Severity::Info);
              },
              "", "Theme")
          .Add(
              "Toast: Info",
              [] { Notify("Info notification", Severity::Info); }, "", "Test")
          .Add(
              "Toast: Success",
              [] { Notify("Build passed!", Severity::Success); }, "", "Test")
          .Add(
              "Toast: Warning",
              [] { Notify("Disk usage high", Severity::Warning); }, "", "Test")
          .Add(
              "Toast: Error",
              [] { Notify("Connection refused", Severity::Error); }, "", "Test")
          .Add(
              "Quit",
              [&] {
                stop = true;
  ...
  root |= Keymap()
              .Bind(
                  ...
                  "Quit")
              .Bind(
                  "h", [] { g_router->Push("home"); }, "Go home")
              .Bind(
                  "Escape", [] { g_router->Pop(); }, "Go back")
              .AsDecorator();
