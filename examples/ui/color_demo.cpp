...
Element AttributesTab() {
  ...
  std::vector<AttrDemo> attrs = {
      ...
      {"Strikethrough", [](TextStyle& s) -> TextStyle& { return s.Strikethrough(); }},
      {"Bold+Dim",      [](TextStyle& s) -> TextStyle& { return s.Bold().Dim(); }},
      {"Bold+Underline",[](TextStyle& s) -> TextStyle& { return s.Bold().Underline(); }},
  };

  Elements rows;
  rows.push_back(
      hbox({text(" Attribute     ") | bold,
            text(" Example             ") | bold}) |
...
Element Colors256Tab() {
  return vbox({
      text("256-color palette (16×16 grid)") | bold | color(Color::YellowLight),
      separator(),
      ColorPalette256(),
  }) | border;
}
...
Component TrueColorTab() {
  ...
  auto preview = Renderer([state] {
    Color c1 = Color::RGB(
        static_cast<uint8_t>(state->r),
        static_cast<uint8_t>(state->g),
        static_cast<uint8_t>(state->b));
    Color c2 = Color::RGB(
        static_cast<uint8_t>(state->r2),
    ...
    return vbox({
        ...
        separator(),
        hbox({text("Red → Blue:   "),
              ColorGradient(Color::RGB(255,0,0), Color::RGB(0,0,255), 40)}),
        hbox({text("Green → Red:  "),
              ColorGradient(Color::RGB(0,255,0), Color::RGB(255,0,0), 40)}),
        hbox({text("Blue → White: "),
              ColorGradient(Color::RGB(0,0,255), Color::RGB(255,255,255), 40)}),
        hbox({text("Custom C1→C2: "),
              ColorGradient(c1, c2, 40)}),
        separator(),
        text("Custom Color 1") | bold,
        hbox({ColorSwatch(c1), text("  RGB("
            + std::to_string(state->r) + ","
            + std::to_string(state->g) + ","
            + std::to_string(state->b) + ")") | color(c1)}),
        text("Custom Color 2") | bold,
        hbox({ColorSwatch(c2), text("  RGB("
            + std::to_string(state->r2) + ","
            + std::to_string(state->g2) + ","
            + std::to_string(state->b2) + ")") | color(c2)}),
    }) | border;
  });

  auto sliders = Container::Vertical({
      slider_r, slider_g, slider_b,
      slider_r2, slider_g2, slider_b2,
  });
  ...
  return Container::Vertical({
      Renderer([markup, input] {
        return vbox({
            ...
            separator(),
            text("Supported tags: [bold] [dim] [italic] [underline] [blink]") | dim,
            text("  [red/green/blue/yellow/cyan/magenta/white/black]") | dim,
            text("  [bright_red] [bg_red] [rgb(r,g,b)] [bg_rgb(r,g,b)] [/]") | dim,
            separator(),
  ...
  return Container::Vertical({
      Renderer([ansi_text, input] {
        return vbox({
            ...
            separator(),
            text("SGR codes: \\033[1m bold  \\033[2m dim  \\033[3m italic") | dim,
