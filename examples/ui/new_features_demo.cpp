...
int main() {
  ...
  auto txt_email = TextInput("Email")
    .Bind(&email_val)
  ...
  auto grid = Grid(3)
    .Gap(1)
    .RowGap(1)
    .CellComponent(btn1).CellComponent(btn2).CellComponent(btn3)
    .CellComponent(btn4).CellComponent(btn5).CellComponent(btn6)
    .Build();
  ...
  auto modal_body = Renderer([&]() -> Element {
    return vbox({
        ...
        paragraph("Are you sure you want to delete this item? "
                  "This action cannot be undone.") | xflex,
        separatorEmpty(),
    });
  });
  ...
  auto drawer_content = Renderer([&]() -> Element {
    ...
    return vbox({
        hbox({ text("  Name:  ") | dim, text(name_val.empty() ? "(empty)" : name_val) | bold }),
        hbox({ text("  Email: ") | dim, text(email_val.empty() ? "(empty)" : email_val) | bold }),
        hbox({ text("  Age:   ") | dim, text(age_val.empty() ? "(empty)" : age_val) | bold }),
        separatorEmpty(),
        text("  (from TextInput above)") | dim | color(t.text_muted),
    });
  });
  auto btn_open_drawer = Button(" Open Drawer", [&]{ show_drawer = !show_drawer; });
  ...
  auto btns_comp   = Container::Horizontal({btn_open_modal, btn_open_drawer,
                                            btn_save, btn_load});
  auto root_layout = Container::Vertical({form_inputs, grid_comp, btns_comp,
                                          modal_body});

  auto body = Renderer(root_layout, [&]() -> Element {
    const Theme& t = GetTheme();
    return vbox({
        text("  ftxui::ui — New Features Demo  ") | bold | hcenter | color(t.primary),
        separatorLight(),
        separatorEmpty(),

        // TextInput section
        text(" TextInput (label · validation · maxlength)") | bold | color(t.secondary),
        separatorEmpty(),
        txt_name->Render(),
        txt_email->Render(),
        txt_age->Render(),
        ...
        separator(),
        separatorEmpty(),
        text(" Grid(3) — 3-column button grid") | bold | color(t.secondary),
        separatorEmpty(),
        grid->Render(),
        separatorEmpty(),
        ...
        separator(),
        separatorEmpty(),
        text(" Modal · Drawer · Theme Persistence") | bold | color(t.secondary),
        separatorEmpty(),
        hbox({
            btn_open_modal->Render(), text("  "),
            btn_open_drawer->Render(), text("  "),
            btn_save->Render(), text("  "),
            btn_load->Render(),
        }) | hcenter,
        separatorEmpty(),
        (modal_result.empty() ? text("") :
            text("  " + modal_result) | color(t.accent)),
        (save_status.empty() ? text("") :
  ...
  auto with_modal = WithModal(with_drawer, "Confirm Delete", modal_body,
      {{"Cancel", [&]{ show_modal = false; modal_result = "Cancelled"; }, false},
       {"Delete", [&]{ show_modal = false; modal_result = "Deleted!";   }, true}},
      &show_modal);

  // Keymap
  with_modal |= Keymap()
    .Bind("q", []{ if (App* a = App::Active()) a->Exit(); }, "Quit")
    .Bind("d", [&]{ show_drawer = !show_drawer; }, "Toggle drawer")
    .AsDecorator();
