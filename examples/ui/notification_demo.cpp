...
int main() {
  ...
  auto btn_info    = Button("[i] Info",    []{
    Notify("Build succeeded in 2.3s", Severity::Info);
  });
  auto btn_success = Button("[✓] Success", []{
    Notify("File saved successfully!", Severity::Success);
  });
  auto btn_warning = Button("[!] Warning", []{
    Notify("Disk usage above 80%", Severity::Warning);
  });
  auto btn_error   = Button("[✗] Error",   []{
    Notify("Connection timed out", Severity::Error);
  });
  ...
  auto layout = Container::Vertical({
      btn_info, btn_success, btn_warning, btn_error, btn_persist,
  });
  ...
  auto body = Renderer(layout, [&]() -> Element {
    ...
    return vbox({
               ...
               paragraph(
                   ...
                   "auto-dismiss after 3 seconds (except Persist).") | xflex,
               separatorEmpty(),
               separatorLight(),
               separatorEmpty(),
