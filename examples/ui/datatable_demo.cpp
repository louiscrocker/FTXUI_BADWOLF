...
int main() {
  ...
  auto table =
      DataTable<Package>()
          .Column(
              "Name", [](const Package& p) { return p.name; }, 12)
          .Column(
              "Version", [](const Package& p) { return p.version; }, 8)
          .Column(
              "License", [](const Package& p) { return p.license; }, 8)
          .Column(
              "Downloads",
              [](const Package& p) { return std::to_string(p.downloads); }, 10)
          .Column(
              "Installed",
              [](const Package& p) { return p.installed ? "✔" : ""; }, 10)
          .Data(&packages)
          .Selectable(true)
          .Sortable(true)
          .AlternateRows(true)
          .OnSelect([&](const Package& p, size_t) { selected_name = p.name; })
