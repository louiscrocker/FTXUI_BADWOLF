...
int main() {
  ...
  std::thread worker([&] {
    ...
    while (!stop) {
      switch (i % 5) {
        case 0:
          log->Log("Server started on port 8080", LogLevel::Info);
          break;
        case 1:
          log->Log("Request GET /api/health (200 OK)", LogLevel::Debug);
          break;
        case 2:
          log->Log("Cache miss — fetching from DB", LogLevel::Trace);
          break;
        case 3:
          log->Log("Response time 142ms", LogLevel::Info);
          break;
        case 4:
          if (i % 15 == 4) {
            ...
            log->Log("Worker processed task #" + std::to_string(i),
                     LogLevel::Debug);
          }
          break;
      }
  ...
  // Filter controls
  std::vector<std::string> level_names = {"Trace", "Debug", "Info", "Warn",
                                          "Error"};
  int level_sel = 0;
