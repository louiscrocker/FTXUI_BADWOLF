#!/usr/bin/env python3
"""
amalgamate.py — Generate a single-header distribution of FTXUI.

Usage: python3 tools/amalgamate.py [--output dist/ftxui.hpp]

The output file can be dropped into any C++ project with no other dependencies.
Users just #include "ftxui.hpp" and link with -lftxui (or use the source directly).

Since FTXUI has .cpp files, the single-header approach works as follows:
  1. Contains ALL public headers inline (with include guards handled)
  2. When FTXUI_IMPLEMENTATION is defined, also includes all .cpp source

Build the generated header:
    g++ -std=c++20 -DFTXUI_IMPLEMENTATION -x c++ dist/ftxui.hpp -c -o ftxui.o
Or for a quick smoke-test:
    g++ -std=c++20 dist/test_amalgamate.cpp -o test_amalgamate
"""
...
BANNER = """\
// =============================================================================
// FTXUI Amalgamated Header — ftxui.hpp
// Generated by tools/amalgamate.py
//
// This file contains the entire FTXUI public API.
// Drop it into your project: #include "ftxui.hpp"
//
// To also compile the implementation in one translation unit:
...
// =============================================================================
// FTXUI Implementation Section
// Only compiled when FTXUI_IMPLEMENTATION is defined.
// =============================================================================
...
def amalgamate(output_path: Path) -> None:
    ...
    for src in sources:
        ...
        content = rewrite_local_includes(content, included)
        # Also rewrite relative includes like #include "../util/..." etc.
        sections.append(f"\n// ---- {rel} ----\n")
        sections.append(content)
    ...
    print(f"Wrote {output_path}  ({lines} lines, {output_path.stat().st_size // 1024} KB)")

    # Generate companion smoke-test file
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate FTXUI single-header amalgamation.")
    parser.add_argument(
